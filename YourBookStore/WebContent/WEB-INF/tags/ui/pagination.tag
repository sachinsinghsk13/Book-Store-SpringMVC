	...
	<ul class="pagination justify-content-center">
		...
		<c:if test="${currentPage gt 1}">
			<li class="page-item"><a class="page-link"
				href="${pageContext.request.contextPath}${baseurl}?page=${currentPage - 1}">Previous</a></li>
		</c:if>
			...
			<c:choose>
				<c:when test="${current eq currentPage}">
					<li class="page-item active"><a class="page-link"
						href="${pageContext.request.contextPath}${baseurl}?page=${current}">${current}</a></li>
				</c:when>
				<c:otherwise>
					<li class="page-item"><a class="page-link"
						href="${pageContext.request.contextPath}${baseurl}?page=${current}">${current}</a></li>
		...
		<c:if
			...
			<li class="page-item"><a class="page-link"
				href="${pageContext.request.contextPath}${baseurl}?page=${currentPage + 1}">Next</a></li>
		</c:if>
	</ul>
</nav>